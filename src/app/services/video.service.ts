...
export class VideoService {
  ...
  getPlaylistVideos = (playlistId: string, bypassCache = false): Observable<{ lastUpdated: number; items: Video[] }> =>
    this.http.get<{ lastUpdated: number; items: Video[] }>(`/api/getPlaylistFeed/${playlistId}?bypassCache=${bypassCache}`).pipe(shareReplay());
