...
loadResource('credentials', true, false, true).then((creds) =>{
  console.log(creds);
  console.log({
    clientID: creds.web.client_id,
    clientSecret: creds.web.client_secret,
    callbackURL: 'http://ytlist.laprell.org:4200/api/auth',
  });
  passport.use(
    new GoogleStrategy(
      {
  ...
  app.get('/api/auth', passport.authenticate('google', { failureRedirect: '/' }),  (req: ExpressRequest, res: ExpressResponse) => {
    console.log('FOO', req, res);   
    res.redirect('/');
    
  
    /*
    loadResource('credentials', true, false, true).then((creds: any) => {
      console.log('CREDS', creds);
      const oauth2Client: OAuth2Client = new OAuth2(creds.installed.client_id, creds.installed.client_secret, creds.installed.redirect_uris[0]);
  
      oauth2Client.getToken(req.query['code']).then((tokenResponse: GetTokenResponse) => {
  ...
  app.get('/api/login', passport.authenticate('google', { scope: SCOPES }), (req: ExpressRequest, res: ExpressResponse) => {
    console.log('BAR', res);
    console.log(req);
  });
